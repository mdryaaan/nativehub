...
<CalloutBox variant="info" title="Why nginx-unprivileged">
  ...
  satisfy `runAsNonRoot: true`. The `nginx-unprivileged` variant runs as UID 101 and listens on 8080
  — the same trade every well-behaved container image makes.
</CalloutBox>
...
If it does not load, check that the Service has endpoints. Empty endpoints means no _ready_ Pod
