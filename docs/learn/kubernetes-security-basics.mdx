...
1. **Authentication** — who are you? Client certificates, bearer tokens, or an OIDC provider. Note
   ...
   ServiceAccounts, by contrast, *are* real objects in the cluster.
...
<CalloutBox variant="tip" title="Binding to port 80 as non-root">
  ...
  is to listen on 8080 in the container and have the Service map `port: 80` to
  `targetPort: 8080` — no capability needed.
</CalloutBox>
