...
- An **image** is an immutable, content-addressed artifact identified by a digest such as
  `sha256:9f3c…`. Tags like `nginx:1.27` are mutable pointers *to* a digest.
...
2. **Cache-friendly ordering.** Files that change rarely (`go.mod`) are copied before files that
   change constantly (source). Docker invalidates a layer *and everything after it*, so putting the
