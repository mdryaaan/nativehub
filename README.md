...
> [!NOTE]
> Local dev serves from `/`, exactly like the deployed site. Only the GitHub Pages workflow builds
> with the `/nativehub/` project subpath, via `GITHUB_PAGES=true`.
...
The same source builds for every target. `baseUrl` is resolved at build time in `docusaurus.config.ts`
...
| GitHub Pages      | `GITHUB_PAGES=true`                 | `mdryaan.github.io`             | `/nativehub/` |
| Local dev         | nothing set                         | production URL                  | `/`           |
...
for fingerprinted assets, and baseline security headers.

This site is deployed at **[nativehub-chi.vercel.app](https://nativehub-chi.vercel.app/)**. To
deploy your own copy, import the repo at [vercel.com/new](https://vercel.com/new) — every setting is
already in `vercel.json`, so accept the detected defaults and deploy.

To check a production build locally first:

```bash
npm run build && npm run serve   # http://localhost:3000/ — identical paths to production
```
